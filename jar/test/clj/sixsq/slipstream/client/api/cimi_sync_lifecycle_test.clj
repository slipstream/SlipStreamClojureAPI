(ns sixsq.slipstream.client.api.cimi-sync-lifecycle-test
  ...
  (:require
    [kvlt.core]
    [sixsq.slipstream.client.api.cimi.sync :as i]
  ...
  (if *server-info*
    (let [{:keys [username password cep-endpoint]} *server-info*]
      ...
      (let [context (i/instance cep-endpoint)
        ...
        (try
          (c/login context {:href     "session-template/internal"
                            :username "UNKNOWN"
                            :password "USER"})
          (is (nil? "function should have thrown exception but did not"))
          (catch Exception e
        ...
        (let [response (c/add context "events" example-event)]
          ...
          (let [event-id (:resource-id response)
            ...
            (is (thrown? Exception (c/edit context event-id read-event)))
