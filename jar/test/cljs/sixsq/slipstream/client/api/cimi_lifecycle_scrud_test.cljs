(ns sixsq.slipstream.client.api.cimi-lifecycle-scrud-test
  ...
  (:require
    [sixsq.slipstream.client.api.cimi-scrud-protocol :as p]
    [sixsq.slipstream.client.api.cimi-async-binding :as binding]
...
;; FIXME: add these correctly into the server-info
(def login-url "http://nuv.la/auth/login")
(def logout-url "http://nuv.la/auth/logout")
    ...
    (go
      (if *server-info*
        (let [{:keys [username password endpoint login-endpoint]} *server-info*]
          ...
          (let [ctx (<! (binding/reify-cimi-async endpoint login-url logout-url))
                cep (<! (p/cloud-entry-point ctx))]
            (is (map? cep))
            (is (:baseURI cep))
            (is (:events cep))

            ;; log into the server
            (let [[status token] (<! (p/login ctx {:username username :password password}))]
              (is (= 200 status))
              (is (string? token))
              (is (not (s/blank? token)))

              ;; search for events
              (let [events (<! (p/search ctx "events"))]
                (is (map? events))
                (is (:count events))

                ;; add a new event resource
                (let [add-event-resp (<! (p/add ctx "events" example-event))]
                  ...
                  (let [event-id (:resource-id add-event-resp)
                        read-event (p/get ctx event-id)
                        events (p/search ctx "events")]
                    (is (= (strip-fields example-event) (strip-fields read-event)))
                    (is (pos? (:count events)))

                    ;; events cannot be edited
                    (is (thrown? js/Error (p/edit ctx event-id read-event)))

                    ;; delete the event and ensure that it is gone
                    (let [delete-resp (p/delete ctx event-id)]
                      (is (= 200 (:status delete-resp)))
                      (try
                        (let [get-resp (p/get ctx event-id)]
                          (is (nil? get-resp)))
                        (catch js/Error ex
                          (let [resp (ex-data ex)]
                            (is (= 404 (:status resp)))))))))))))))
        ...
        (let [{:keys [username password endpoint login-endpoint]} *server-info*]
          ...
          (let [ctx (<! (binding/reify-cimi-async endpoint login-url logout-url))
            ...
            (let [[status token] (<! (p/login ctx {:username username :password password}))]
              ...
              (let [attrs (p/search ctx "serviceAttributes")]
                #_(is (map? attrs))
                (is (:count attrs))

                ;; add a new attribute resource
                (let [add-attr-resp (<! (p/add ctx "serviceAttributes" example-attr))]
                  ...
                  (let [attr-id (:resource-id add-attr-resp)
                        read-attr (<! (p/get ctx attr-id))
                        attrs (<! (p/search ctx "serviceAttributes"))]
                    ...
                    (let [updated-attr (assoc read-attr :major-version 10)
                          edit-resp (<! (p/edit ctx attr-id updated-attr))
                          reread-attr (<! (p/get ctx attr-id))]
                      #_(is (map? edit-resp))
                    ...
                    (let [delete-resp (<! (p/delete ctx attr-id))]
                      (is (= 200 (:status delete-resp)))
                      (try
                        (let [get-resp (<! (p/get ctx attr-id))]
                          (is (nil? get-resp)))
                        (catch js/Error ex
                          (let [resp (ex-data ex)]
                            (is (= 404 (:status resp)))))))))))))))
